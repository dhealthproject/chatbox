...
export interface SolanaCommerceConfig {
    ...
    /** API key (or id) — a unique nonce prefix is added per payment at runtime */
    readonly paymentMemo?: string;
    /** When set, only these amounts are selectable per currency (hides custom amount) */
    readonly fixedAmounts?: Partial<Record<Currency, number>>;
    /** Token logo URLs for the payment panel (use data: or https: URLs for iframe CSP) */
    readonly currencyLogos?: Partial<Record<Currency, string>>;
}
...
export type Currency = 'AIDH' | 'USDC' | 'SOL' | 'USDT' | 'USDC_DEVNET' | 'SOL_DEVNET' | 'USDT_DEVNET';
...
export const CurrencyMap: Record<Currency, SPLTokenInfo | 'SOL'> = {
    ...
    // Mainnet AIDH token
    AIDH: {
        mint: address('AidHczUkwDnW7c1Lc89tTiP71dTqeEa52LgV6GxsfwYd'),
        tokenProgram: TOKEN_PROGRAM_ADDRESS,
        decimals: 6,
        symbol: 'AIDH',
        name: 'dHealth Intelligence (AIDH)',
    },
