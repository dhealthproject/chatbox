...
import type { ThemeConfig, BorderRadius, CommerceMode, MerchantConfig } from '@solana-commerce/kit';
...
/** Format a token amount with locale grouping and ticker suffix (e.g. "20 USDC", "2,000 AIDH"). */
export const formatTokenAmount = (amount: number, currency: Currency): string => {
    if (amount <= 0) {
        return `0 ${currency}`;
    }
