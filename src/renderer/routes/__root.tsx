...
import { buildDementiaAgentPrompt } from '@/utils/dementia-demo'
import { buildNutrientAgentPrompt } from '@/utils/nutrient-demo'
...
const demoCheck = async(
  ...
  const [
    pgValue, phValue, relValue, livValue, nameValue, ageValue, dtypeValue, intensityValue, langValue,
    cValue, tValue, sValue, addrValue, expValue, exValue, escValue, telValue, dValue, refValue, hcpValue, nextValue, tasksValue, xrValue, hrsValue, warnValue
  ] = params.map(
  ...
  if (pgValue && phValue && relValue && livValue) {
    ...
    prompt = buildDementiaAgentPrompt(
      pgValue,
      phValue,
      relValue,
      livValue,
    ...
    const taskValues = tasksValue?.split(',') ?? []
    const exValues = exValue?.split(',') ?? []

    prompt = buildNutrientAgentPrompt(
      cValue,
      tValues,
      sValue,
      addrValue,
